<div wire:poll>
    ...
    @endif
    {{-- start create post --}}
    @if($isFollower or $author)
        <div class="card w-100 shadow-xss rounded-xxl border-0 pe-4 pt-4 ps-4 pb-3 mb-3">
            <div class="card-body p-0">
                <p class="font-xssss fw-600 text-grey-500 card-body p-0 d-flex align-items-center"><i
                        class="btn-round-sm font-xs text-primary ms-2 bg-greylight"> <span style="margin-top:-7px;"><?php echo $icons->getIcon('edit'); ?></span></i>Create
                    Post</p>
            </div>
            <div style="margin-left:50px;margin-bottom:10px;" class="card-body p-0 mt-3 position-relative">
                <a href="{{ route('user-profile',auth()->user()->uuid ) }}">
                    ...
                    <figure class="avatar position-absolute me-4 mt-1 top-5"><img
                            style="border-radius: 100%;width:50px;height:50px;"
                            src="{{ asset('storage\\').auth()->user()->profile  ?? '../images/profile-2.png'}}" alt="image"
                            class="shadow-sm">
                    </figure>
                </a>
                <textarea wire:model='message' name="message" style="margin-right: 25px"
                          class="h100 bor-0 w-100 rounded-xxl p-2 pe-5 font-xssss text-grey-900 fw-500 border-light-md theme-dark-bg"
            ...
            <div class="card-body d-flex p-0 mt-0">
                ...
                <a href="#"
                   class="uploads d-flex align-items-center font-xssss fw-600 ls-1 text-grey-700 text-dark ps-4">
                ...
                <button wire:click.prevent='storePost' title="add new post" onclick="window.scrollTo(0,0)"
                        style="background-color:rgb(101, 11, 236)" class="me-auto btn text-white rounded-circle"> <span style="margin-top:-7px;"><?php echo $icons->getIcon('send'); ?></span></i></button>
